...
def light_source_intensity_vecterize(r_array,t_array,N_Rs,solar_simulator_settings, light_source_property):
    ...
    #q = np.zeros((len(t_array),len(r_array)))
    
    I = bs+ks*V_DC+ks*As*np.sin(2*np.pi*f_heating*t_array[:,np.newaxis]) # t: time, r: distance from center
...
def radial_1D_explicit(Fo_criteria, vecterize, sample_information, vacuum_chamber_setting, solar_simulator_settings,light_source_property, numerical_simulation_setting):
    ...
    if vecterize:
        ...
        for p in range(Nt-1): #p indicate time step
            ...
            T[p+1,1:-1] = T[p,1:-1]+Fo_r*(rm[1:-1])/rm[1:-1]*(T[p,0:-2]-T[p,1:-1])+Fo_r*(rm[1:-1])/rm[1:-1]*(T[p,2:]-T[p,1:-1]) \
            + dt/(rho*(cp_const + cp_c1*T[p,1:-1]+cp_c2*T[p,1:-1]**2+cp_c3*T[p,1:-1]**3)*dz)*(absorptivity*sigma_sb*T_sur1**4+absorptivity*sigma_sb*T_sur2**4+absorptivity*q_solar[p,1:-1]-2*emissivity*sigma_sb*T[p,1:-1]**4)        
...
def simulation_result_amplitude_phase_extraction(df_amplitude_phase_measurement,sample_information, vacuum_chamber_setting, solar_simulator_settings,light_source_property, numerical_simulation_setting):
    ...
    T_,time_T_, r_ = radial_1D_explicit(1/3,numerical_simulation_setting['vecterize'],sample_information, vacuum_chamber_setting, solar_simulator_settings,light_source_property, numerical_simulation_setting)
